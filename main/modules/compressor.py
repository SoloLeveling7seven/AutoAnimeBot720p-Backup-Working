...
async def gg():
  cmd = 'ffmpeg -hide_banner -loglevel quiet -progress "progressaa.txt" -i "video.mkv" -metadata title="Encoded By Anime Recommendation" -vf subtitles=video.mkv -preset faster -c:v libx264 -speed 2 -crf 25.8 -map 0:v -c:a aac -map 0:a -c:s copy -map 0:s? "out.mkv" -y'  
